          ...
          return request(app)
            ...
            .then(({ body }) => {
              ...
              expect(body.msg).to.equal(
                'No user found for username: xylophone'
              );
            });
        });
      });
      ...
      describe('GET', () => {
        ...
        it('status:200 filters the articles by the username specified in the query', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(
                _.every(body.articles, ['author', 'icellusedkars'])
              ).to.equal(true);
        ...
        it('status:200 succesfully handles all valid querystring options', () => {
          return request(app)
            .get(
              '/api/articles?topic=mitch&author=rogersop&sort_by=title&order=asc'
            )
            .expect(200)
            .then(({ body }) => {
        ...
        it('status:400 when order is neither "asc" / "desc"', () => {
          // i'm handling this as a custom error, because the in knex, it ignores an invalid sort_by. so sort_by 'banana' is ignored, and it just returns sorted by default 'desc', giving a status 200 OK. NO GOOD. I wanted to handle it (my choice of the 2 schools of thought as described by mitch.) --> so, I made a custom error.
          return request(app)
        ...
        it('status:404 author that is not in the database', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `No articles found - author 'xylophone' does not exist.`
              );
            });
        });
        it('status:404 topic that is not in the database', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `No articles found - topic 'formicophilia' does not exist.`
              );
            });
        });
        it('status:404 given both topic and author but neither exist in the database', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `No articles found - author 'gorbachev' and topic 'chernobyl' do not exist.`
              );
            });
        });
        it('status:404 author exists but does not have any articles associated with it', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `No articles found - author 'lurker' hasn't written any yet.`
              );
            });
        });
        it('status:404 topic exists but does not have any articles associated with it', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `No articles found - none currently exist on the topic of 'paper'. Be the first to write one!`
              );
            });
        });
        it(`status:404 both author and topic exist, but there aren't any articles associated with either`, () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `No articles found - 'lurker' hasn't written any articles, and 'paper' has no articles for it.`
              );
            });
        });
      });
          ...
          it('status:404 when resource does not exist', () => {
            return request(app)
              ...
              .then(({ body }) => {
                expect(body.status).to.equal(404);
                expect(body.msg).to.equal(
                  'No article found with article_id 99999'
                );
              });
          });
        });
            ...
            it('status:200 responds with the comments for a given article when article has multiple comments', () => {
              return request(app)
                ...
                .then(({ body }) => {
                  ...
                  expect(body.comments[12]).to.have.keys(
                    'comment_id',
                    'author',
                    'article_id',
                    'votes',
            ...
            it("status:404 when article_id doesn't exist", () => {
              return request(app)
                ...
                .then(({ body }) => {
                  expect(body.status).to.equal(404);
                  expect(body.msg).to.equal(
                    "Article with article_id 9999 doesn't exist."
                  );
                });
            });
          });
        ...
        it('status:404 when comment_id is valid, but no such comment exists', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `No comment found with comment_id 1000`
              );
            });
        });
        ...
        it('status:400 Invalid inc_votes value', () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(400);
              expect(body.msg).to.equal(
                `inc_votes of 'cat' is invalid - please provide an integer.`
              );
            });
        });
        ...
        it(`status:400 Some other property on request body `, () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(400);
              expect(body.msg).to.equal(
                `Invalid additional keys on the request object - please only provide inc_votes.`
              );
            });
        });
      });
        ...
        it("status:404 if trying to delete a comment that doesn't exist", () => {
          return request(app)
            ...
            .then(({ body }) => {
              expect(body.status).to.equal(404);
              expect(body.msg).to.equal(
                `Cannot delete - no comment exists with comment_id 99999`
              );
            });
        });
      });
